...
func _ready() -> void:
	...
	var linear := (load("res://src/world/moving_platform.tscn") as PackedScene).instantiate() as MovingPlatform
	linear.travel = Vector2(480, 0)
	linear.speed = 180.0
	add_child(linear)
	var orbit := (load("res://src/world/moving_platform.tscn") as PackedScene).instantiate() as MovingPlatform
	orbit.motion_mode = MovingPlatform.MotionMode.ORBIT
	orbit.orbit_radii = Vector2(220, 110)
	add_child(orbit)
	var blink := (load("res://src/world/blink_platform.tscn") as PackedScene).instantiate() as BlinkPlatform
